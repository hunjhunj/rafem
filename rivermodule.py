...
class RiverModule(object):
    def __init__(
        ...
        savesplaydeposit=False,
#        savefloodplain=True,
        FP_couple=1.0,
        FP_decay=0.03,
        lamb = 1.0, #wash load ratio
        omeg = 1.0, #channel sinuosity 
        drop_percent = 0.5,
        backwater = True,
        dcell = 5,
        ...
        self._dcell = dcell
 #       self._ch_discharge = ch_discharge
        ...
        self._FP_RC_couple = FP_couple
        self._FP_depo_decay = FP_decay
        ...
        self._save_splay_deposit = savesplaydeposit
        self._backwater = backwater
#        self._savefloodplain = savefloodplain
#        self._floodplainfile = os.path.abspath("../output_data/floodplain.out") #
...
#        if self._savefloodplain:
#            make_empty_file(self._floodplainfile) 
    ...
    def advance_in_time(self):
        ...
        # if (self._time / _SECONDS_PER_DAY) > 16425:
        #     print ("initial:",self._riv_i[-1], self._riv_j[-1])
        ...
        # if (self._time / _SECONDS_PER_DAY) > 16425:
        #     print ("after course update:",self._riv_i[-1], self._riv_j[-1])
        ...
        # if (self._time / _SECONDS_PER_DAY) > 16425:
        #     print ("after avulsion:",self._riv_i[-1], self._riv_j[-1]) 
            ...
            with open(self._avulsionfile, "a") as file:
                file.write(
                    ...
                    % (
                        ...
                        self._riv_i[-1],
                        self._riv_j[-1],
                        self._riv_i[self._loc],
                        self._riv_j[self._loc],
                    )
                )

#         """ Save floodplain aggradation. """
#         if self._savefloodplain:
#             with open(self._floodplainfile, "a") as file:
#                 file.write(
...
#         FP.dep_fines(
#             self._n, self._riv_i, self._riv_j, self._dn_rc, self._SL, self._FP_RC_couple, self._FP_depo_decay
#         )        
    
        self._depo_decay, self._eff_width = FP.dep_decay(
            self._n, self._riv_i, self._riv_j, self._dn_rc, self._SL, self._FP_RC_couple, self._FP_depo_decay, self._dx, self._ch_width,
        )        
...
#        T3 = time.process_time()
        ...
        if self._backwater == False: 
            self._dn_rc = diffuse.smooth_rc2(
                self._dx,
                self._dy,
                self._nu,
                self._dt,
            ...
            self._dn_rc = diffuse.smooth_rc3(
                self._dx,
                self._dy,
                self._nu,
                self._dt,
        ...
        # if (self._time / _SECONDS_PER_DAY) > 16425:
        #     print ("after river diffuse:",self._riv_i[-1], self._riv_j[-1]) 
